...
import os, asyncio, json, binascii, requests, aiohttp, urllib3
...
def get_info(enc, server, token):
    ...
    try:
        p = like_count_pb2.Info(); p.ParseFromString(r.content); return p
    except DecodeError:
        return None
...
def like():
    ...
    server = (request.args.get("server") or "").upper()
...
if __name__ == "__main__":
    # IMPORTANT for Render: bind to 0.0.0.0 and use PORT from env
